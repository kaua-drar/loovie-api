/*
  ...
  - You are about to drop the column `language_id` on the `TitleTranslation` table. All the data in the column will be lost.
  ...
  - Added the required column `language_code` to the `TitleTranslation` table without a default value. This is not possible if the table is not empty.
...
-- AlterTable
ALTER TABLE "TitleTranslation" DROP COLUMN "language_id",
ADD COLUMN     "language_code" TEXT NOT NULL;
...
-- AddForeignKey
ALTER TABLE "TitleTranslation" ADD CONSTRAINT "TitleTranslation_language_code_fkey" FOREIGN KEY ("language_code") REFERENCES "Language"("code") ON DELETE RESTRICT ON UPDATE CASCADE;
